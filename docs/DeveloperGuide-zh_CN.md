...
> 项目介绍、特性与下载方式请见 [README](https://github.com/CrazySue/Checklist)。  
...
⚠️ 凡涉及动画的状态变化必须**直接操作 DOM**，不要触发整体重渲染。
...
- **咖啡态**：出现/退出用 `@keyframes`（Android 可靠）；检查单切换时视为"特殊检查项"随滚动区翻动（`instant` 路径禁用自身动画）；
...
| 咖啡态 | 切换时随滚动区翻动；重置时向下移出+淡出 | 统一翻页观感 |
...
| 0.8.0 | 初版评审基线                                                                    |
| 0.9.0 | 底栏聚焦、咖啡态翻页、编辑页导航、图标分类本地化、200+ 关键词/语言、.checklist 导出、赞助按钮、水波裁剪、横屏左栏、嵌入 Bold |
| 0.9.5 | 翻页方向规则、编辑↔新建翻动、开关规格、自动空白行、标题位移动画                                          |
| 0.9.6 | 设置返回方向、安卓导出/导入、标题上下翻动、深色过渡、横屏判定重写                                         |
| 0.9.7 | 标题动画时序、咖啡淡出、空行裁剪、开关回归 0.8.0、署名合并                                          |
