...
(async ()=>{
  ...
  check('settings 从右侧起滑（40px）', $('#pageSettings').style.transform === 'translateX(40px)');
  await sleep(500);
  ...
  check('settings 再次从右侧起滑（40px）', $('#pageSettings').style.transform === 'translateX(40px)');
  await sleep(500);
  ...
  const titleWrap = $('#topbarTitle');
  check('整个标题向上翻动', titleWrap.style.transform.indexOf('translateY') >= 0);
  await sleep(600);
  check('title c2', text('#topbarTitleText') === '工作');
  check('title settled', titleWrap.style.transform === '' || titleWrap.style.transform === 'translateY(0px)');
  // c2 -> c1（左侧）：整个标题向下翻
  d.querySelector('.nav-tab[data-id="c1"]').click();
  await sleep(150);
  check('整个标题向下翻动', titleWrap.style.transform.indexOf('translateY') >= 0);
  await sleep(600);
  ...
  // ---- #4 赞助按钮标题颜色与其他一致 ----
  check('sc-title 与 si-title 完全一致', html.indexOf(".support-card .sc-title{font-size:16px;color:var(--md-on-surface);}") >= 0);

  // ---- #2 外链只开一个标签页（锚点方式） ----
  check('openExternal anchor-only (no window.open call)', html.indexOf('window.open(') < 0);

  // ---- #8 APK 导出到 Download 目录 + 导入任意文件类型 ----
  check('export uses PRIVATE_DOC', html.indexOf('plus.io.PRIVATE_DOC') >= 0);
  check('安卓导入不带 accept 过滤', html.indexOf('isAndroidApp') >= 0);
