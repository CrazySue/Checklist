...
(async ()=>{
  ...
  await sleep(450);
  inputs = d.querySelectorAll('#formContent .form-item input');
  // 新规则：首项清空保留，但多余的空自动行被裁剪（仅剩 1 行）
  check('first item clear trims auto row (1 row left)', inputs.length === 1);
  ...
  await sleep(60);
  check('home 从左起滑（-40px）', d.querySelector('#pageHome').style.transform === 'translateX(-40px)');
  await sleep(300);
